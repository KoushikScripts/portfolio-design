  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-[#03001427] backdrop-blur-md shadow-lg shadow-[#2A0E61]/50 z-50">
      <div className="flex items-center justify-between h-[70px] px-5 md:px-10">
        ...
        {/* Desktop Navigation */}
        <div className="hidden md:flex items-center gap-8 bg-[rgba(3,0,20,0.37)] px-8 py-2 rounded-full border border-[rgba(230,57,70,0.38)] text-gray-200">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.title}
              href={link.link}
        ...
        <div className="hidden md:flex items-center gap-5">
          {SOCIALS.map(({ link, name, icon: Icon }) => (
            ...
            >
              <Icon className="h-6 w-6 text-white hover:text-[#E63946] transition" />
            </Link>
          ))}
        </div>
            ...
            <div className="flex space-x-6 mt-10">
              {SOCIALS.map(({ link, name, icon: Icon }) => (
                <Link
                  ...
                  className="flex items-center"
                >
                  <Icon className="h-8 w-8 text-white hover:text-[#E63946] transition" />
                </Link>
              ))}
            </div>
          </motion.div>
